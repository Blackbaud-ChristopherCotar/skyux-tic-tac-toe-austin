...
describe("GameRepository", () => {
    it("should return and empty list when new", () => {
    ...
    it("should create a game with a human as the first player", () => {
        let repo = new GameRepository();
        let now = new Date();
        ...
        newGame.startedOn.should.be.greaterThanOrEqual(now);
    ...
    it("should load saved games from the file system", () => {
        let data = [
            new Game(1, true),
            new Game(2, false)
        ];
    ...
    it("should make a move for the computer when the human is the second player", () => {
        let repo = new GameRepository();
        let now = new Date();

        let newGame = repo.addGame(false);

        //let sum = newGame.board.recuce()
    })
});
